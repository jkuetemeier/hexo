...
var languageGetter = function(){
  return hexo.util.language.get(this.path, this.lang)
};
...
Post.virtual('language', languageGetter);
...
Post.virtual('path', function(){
  ...
  // remove hidden '_' directories from path
  var slug = this.slug
    .replace(/^_.*?\//, '') // first position
    .replace(/\/_.*?\//g, '/'); // middle position
  ...
  var url = permalink
    ...
    .replace(':language', hexo.util.language.prefix(this.language))
    .replace(':title', slug);
...
Page.virtual('language', languageGetter);
