  ...
  async.waterfall([
    ...
    function(data, next){
      ...
      if (config.hasOwnProperty('multi_language') &&
        ...
        config.multi_language.languages.forEach(function(l) {
          if (config.multi_language.hasOwnProperty(l)) {
            ...
            if (config.multi_language[l].path) {
              path = config.multi_language[l].path;
            }
            path = path + '/';

            if (locals.path.indexOf(path) == 0) {
              language = l
            }
          }
        })
        ...
        if (locals.page.language) {
          language = locals.page.language;
        }

        // reconfigure i18n with new post/page language
        locals.__ = locals.themei18n.get(language);  
        locals._p = locals.themei18n.plural(language);

        // load moment language module, see http://momentjs.com/docs/
        locals.moment.lang(language);

        locals.language = locals.lang = language
